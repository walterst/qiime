...
__credits__ =  ["William Walters", "Rob Knight", "Micah Hamady", "Greg Caporaso", "Kyle Bittinger","Jesse Stombaugh","Jens Reeder"] #remember to add yourself
__license__ = "GPL"
...
script_info['script_usage'].append(("""""","""Run demultiplex_fasta on Run 2:""","""%prog -m Mapping_File2.txt -f 2.TCA.454Reads.fna -q 2.TCA.454Reads.qual -o demultiplexed_output_Run2/ -n 2000000"""))
...
script_info['script_usage'].append(("""""","""To use the run prefix at the beginning of the fasta label for demultiplexing, there has to be a field in the mapping file labeled "run_prefix", and can be used by the following command:""","""%prog -m Mapping_File_run_prefix.txt -f 1.TCA.454Reads.fna -q 1.TCA.454Reads.qual -o demultiplexed_output_run_prefix/ -j run_prefix"""))
...
script_info['optional_options']=[\
    ...
    make_option('-o', '--dir_prefix', default='.',
...
def main():
    ...
    fasta_files = opts.fasta_fnames.split(',')
    ...
    if opts.qual_fnames:
        qual_files = opts.qual_fnames.split(',')
    else:
        qual_files = []
